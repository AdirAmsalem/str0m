...
/// Max VP9 RTP payload descriptor size for non-flexible mode, reserved for MTU headroom.
/// flags(1) + 15-bit PID(2) + L byte(1) + tl0picidx(1) + SS(≤1 on keyframe first-packets, G=0) = 6;
/// kept at 8 as a conservative reserve so a fragment never exceeds the MTU.
...
impl Vp9Packetizer {
    ...
    fn packetize_non_flexible(
        ...
        while payload_data_remaining > 0 {
            ...
            if is_keyframe && is_first {
                // N_S(3)|Y(1)|G(1)|RES(3) = N_S=0 (1 spatial layer), Y=0, G=0 (no GOF).
                //
                // A single spatial + single temporal layer stream has no temporal-prediction
                // pattern to describe, so it must NOT advertise a GOF. The previous code emitted
                // G=1 with a 1-entry GOF whose R=0 ("this frame references 0 pictures"), which
                // directly contradicts the P=1 flag set on inter frames above and the actual
                // IPPP bitstream. Receivers (libwebrtc's ParseSsData) take the GOF template as
                // authoritative → conclude frames are reference-less → mis-track references →
                // picture corruption even at 100% packet delivery. With G=0 there is no GOF and
                // the receiver infers references from the P flag + tl0picidx (RFC 7741 §4.2),
                // which is the correct single-layer semantic and enables PLI-based loss recovery.
                out.push(0x00);
            }
